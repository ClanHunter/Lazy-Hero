...
local function extractKeyFromRaw(raw)
  ...
  -- numeric index first element
  if raw[1] then return raw[1] end
  -- shallow search for string or number values
...
function HekiliAdapter:GetSuggestion(unit)
  ...
  if type(tbl.DisplayPool) == "table" and #tbl.DisplayPool > 0 then
    ...
    if type(first) == "table" then
      ...
      if OneButtonAssistantLogger and OneButtonAssistantLogger.Log then pcall(OneButtonAssistantLogger.Log, OneButtonAssistantLogger, "hekili_display", out) end
      return out
    end
  end
  ...
  if type(tbl.State) == "table" then
    ...
    if type(pool) == "table" then
      ...
      if first and type(first) == "table" then
        ...
        if OneButtonAssistantLogger and OneButtonAssistantLogger.Log then pcall(OneButtonAssistantLogger.Log, OneButtonAssistantLogger, "hekili_state", out) end
        return out
      end
    end
  end
  ...
  if type(tbl.Queue) == "table" then
    ...
    if first and type(first) == "table" then
      ...
      if OneButtonAssistantLogger and OneButtonAssistantLogger.Log then pcall(OneButtonAssistantLogger.Log, OneButtonAssistantLogger, "hekili_queue_table", out) end
      return out
    end
  end
    ...
    if type(f) == "function" then
      ...
      if ok and res and type(res) == "table" then
        ...
        if key then
          ...
          if OneButtonAssistantLogger and OneButtonAssistantLogger.Log then pcall(OneButtonAssistantLogger.Log, OneButtonAssistantLogger, "hekili_api", out) end
          return out
        end
      end
    end
...
function HekiliAdapter:Probe()
  -- Non-invasive probe: record field types and simple values but DO NOT call functions
