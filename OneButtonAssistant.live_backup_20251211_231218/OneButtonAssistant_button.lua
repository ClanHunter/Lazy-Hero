  ...
  if name == "suggestion" then
    ...
    if value and value ~= "" then
        ...
        if dbg then
          pcall(function() print(string.format("OneButtonAssistant: SuggestionApplied -> %s (time=%.3f)", tostring(self.__suggestion), (GetTime and GetTime() or 0))) end)
              ...
              pcall(function()
                ...
                table.insert(OneButtonAssistantDB.assignLog, { action = "observed_apply", suggestion = tostring(self.__suggestion), time = (GetTime and GetTime() or 0) })
              end)
        end
      end)
      ...
      pcall(function()
        OneButtonAssistantDB = OneButtonAssistantDB or {}
        OneButtonAssistantDB.assignLog = OneButtonAssistantDB.assignLog or {}
        table.insert(OneButtonAssistantDB.assignLog, { action = "observed_clear", time = (GetTime and GetTime() or 0) })
      end)
    end
  end
end)
...
function OBA:SetButtonSuggestion(suggestion)
  ...
  if not val then
    ...
    pcall(function()
      OneButtonAssistantDB = OneButtonAssistantDB or {}
      OneButtonAssistantDB.assignLog = OneButtonAssistantDB.assignLog or {}
      table.insert(OneButtonAssistantDB.assignLog, { action = "clear", time = (GetTime and GetTime() or 0) })
    end)
