...
function M.probeTable(name, tbl, found)
  local result = { name = name or 'unknown', found = found and true or false, fields = {} }
  if not tbl then return result end
  for k, v in pairs(tbl) do
    local t = type(v)
    result.fields[k] = { type = t, sample = (t == 'string' or t == 'number') and tostring(v) or t }
  end
  return result
end
...
function M.adapter_probe(name)
  ...
  return M.probeTable(name, tbl, type(tbl) == 'table')
end
...
function M.matchToNDJSON(match)
  ...
  local lines = {}
  if M.format_meta_line then table.insert(lines, M.format_meta_line(match.meta or {}, match.stats or {})) end
  for _, e in ipairs(match.events or {}) do
    table.insert(lines, M.format_event_line(e) or '{}')
  end
...
function M.prepare_export_chunks(matchID, linesPerChunk)
  ...
  for _, m in ipairs(OneButtonAssistantDB.bgLogs or {}) do
    if m.meta and m.meta.matchID == matchID then
      ...
      OneButtonAssistantDB.bgExportChunks[matchID] = chunks or {}
      return chunks
    end
  end
  return nil
...
function M.get_player_position()
  ...
  if ok3 and ux and uy then return { x = tonumber(string.format('%.4f', ux)), y = tonumber(string.format('%.4f', uy)), z = tonumber(string.format('%.2f', uz or 0)) } end
  return nil
end

function M.gather_player_meta()
  local safe = M.safeCall
  local name = safe and safe(UnitName, 'player') or 'player'
  local _, class = safe and safe(UnitClass, 'player') or {nil, 'UNKNOWN'}
  local race = safe and safe(UnitRace, 'player') or 'UNKNOWN'
  local level = safe and safe(UnitLevel, 'player') or 0
  local faction = safe and safe(UnitFactionGroup) or 'unknown'
  local guid = safe and safe(UnitGUID, 'player')
  local player = { name = name or 'player', class = class or 'UNKNOWN', race = race or 'UNKNOWN', level = level, faction = faction, guid = guid and M.guidHash(guid) or '-' }
  local members = safe and safe(GetNumGroupMembers) or 0
  local comp = {}
  if members and members > 0 then
    for i = 1, members do
      local unit = (IsInRaid and IsInRaid()) and ('raid'..i) or ('party'..i)
      local _, c = safe and safe(UnitName, unit), safe and safe(UnitClass, unit)
