    ...
    if sourceGUID == state.GUID then
        ...
        elseif spellID == 344179 then
            ...
            elseif subtype == "SPELL_AURA_APPLIED" or subtype == "SPELL_AURA_APPLIED_DOSE" or subtype == "SPELL_AURA_REFRESH" then
                -- Defensive: msw_aura may be nil due to timing between CLEU and aura tracking.
                local NewCount = 0
                if msw_aura then
                    if type(msw_aura.applications) == 'number' then
                        NewCount = msw_aura.applications
                    elseif type(msw_aura.count) == 'number' then
                        NewCount = msw_aura.count
                    end
                end
